...
HISTORY="${OUTPUT_DIR}/was_diag_history_${LABEL}_${TS_FILE}.txt"

# ── 저장소 ──────────────────────────────────────────────────
F_CODE=(); F_SEV=(); F_NAME=(); F_CAT=(); F_FILE=(); F_RAW=(); F_RESULT=(); F_SUMMARY=(); F_STD=(); F_FIX=()
...
# ── KISA 조치 방법(원문) ─────────────────────────────────────
#  2026 주요정보통신기반시설 상세가이드 - 웹 서비스 개요 표의 '조치 방법' 칸 원문만 하드코딩.
#  (CSV '조치방법' 필드용 — 점검·조치 사례 Step 절차는 제외, 가공/요약 없음)
declare -A FIX
FIX[WEB-01]="기본 관리자 계정명을 추측하기 어려운 계정명으로 설정"
FIX[WEB-02]="복잡도 기준에 맞는 추측하기 어려운 비밀번호 설정"
FIX[WEB-03]="비밀번호 파일 권한 600 이하로 설정"
FIX[WEB-04]="디렉터리 리스팅 기능 차단 설정"
FIX[WEB-05]="CGI 스크립트를 정해진 디렉터리 내에서만 실행할 수 있도록 설정"
FIX[WEB-06]="상위 디렉터리 접근 기능 제거 설정"
FIX[WEB-07]="불필요한 파일 및 디렉터리를 제거하도록 설정"
FIX[WEB-08]="파일 업로드 및 다운로드 용량을 허용 가능한 최소 범위로 제한하여 설정"
FIX[WEB-09]="웹 서비스 프로세스 구동 시 관리자 권한이 아닌 운영에 필요한 최소한의 권한을 가진 계정으로 구동 설정"
FIX[WEB-10]="불필요한 Proxy 설정 존재 여부 점검 및 제한 설정"
FIX[WEB-11]="웹 서버의 경로를 별도의 경로로 변경 및 불필요한 경로 제거 설정"
FIX[WEB-12]="웹 서비스 링크 사용 제한 설정"
FIX[WEB-13]="DB 연결 파일에 대한 접근 권한 제한 또는 불필요한 스크립트 매핑 제거 등을 통한 웹 서비스 내 DB 연결 취약점 제거 설정"
FIX[WEB-14]="주요 설정 파일 및 디렉터리에 불필요한 접근 권한 제거 설정"
FIX[WEB-15]="불필요한 스크립트 매핑 존재 여부 점검 및 제거 설정"
FIX[WEB-16]="응답 헤더에 표시되는 정보를 최소한으로 제한하여 설정"
FIX[WEB-17]="불필요한 가상 디렉터리 존재 여부 점검 및 삭제하도록 설정"
FIX[WEB-18]="WebDAV 서비스 비활성화 설정"
FIX[WEB-19]="웹 서비스 내 불필요한 SSI 사용 제한 설정"
FIX[WEB-20]="웹 서비스 내 SSL/TLS 활성화 설정"
FIX[WEB-21]="HTTP Redirection 활성화 설정"
FIX[WEB-22]="필수 에러 코드에 대해 일원화된 에러 페이지 사용 및 에러 페이지 내 불필요 정보 노출 제한 설정"
FIX[WEB-23]="LDAP 연결 인증 시 SHA-256 이상의 알고리즘을 사용하도록 설정"
FIX[WEB-24]="기본 경로가 아닌 별도의 업로드 경로를 지정하고, 해당 경로에 대한 일반 사용자의 접근 권한을 제한하도록 설정"
FIX[WEB-25]="패치 적용에 따른 서비스 영향 정도를 정확히 파악하여 주기적인 패치 적용 정책 수립 및 적용하도록 설정"
FIX[WEB-26]="로그 디렉터리 및 파일에 일반 사용자 접근 권한 제거 설정"
...
record() {
  ...
  F_FIX[i]="${FIX[$code]:-(가이드 조치 방법 미정의)}"
...
diag_web24() {
  ...
  if [ "${#paths[@]}" -eq 0 ]; then
    if [ -d "${APP_CONTEXT_DIR}/upload" ]; then
      ...
      record "WEB-24" "중" "$nm" "$R_PASS" "$srcfile" "(로컬 업로드 경로 미설정 — 외부 저장소(S3 등) 추정)" "로컬 업로드 경로 미설정(외부 저장 추정 — 수동 확인 권장)"
    fi
    return
  fi
