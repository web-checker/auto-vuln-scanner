...
export AWS_RETRY_MODE=adaptive   # API 스로틀(Rate exceeded) 시 CLI 지수백오프 자동 재시도
export AWS_MAX_ATTEMPTS=10        # 최대 시도 횟수(기본 3~4 → 10)로 상향, 부분실패 오판 방지
...
# ── 신뢰성 배관: 파일 기반(명령치환 $() 서브셸에서도 상태 유지) ──
#   aws_ro/aws_ro_cached 는 항상 x=$(aws_ro ...) 형태로 서브셸에서 불리므로
#   배열/변수 누적은 부모로 전파되지 않는다 → temp 파일/디렉터리로 우회.
_DENIED_FILE="$(mktemp)"    # 점검 불가(권한부족/스로틀) API 누적 기록
_AWS_ERRFILE="$(mktemp)"    # 직전 호출의 stderr 캡처(권한오류 판별용)
_RO_CACHE_DIR="$(mktemp -d)" # 동일 호출 결과 캐시(중복 describe 제거)

# 읽기전용 AWS CLI 래퍼: describe/list/get/lookup/generate-credential-report 만 통과
#   ※ 권한부족(AccessDenied 등)/스로틀로 실패하면 stdout은 비지만 _DENIED_FILE에 기록해
#     "리소스 없음/양호"와 "못 봤음"이 혼동되지 않게 함(예전엔 2>/dev/null로 삼켜 오판).
aws_ro(){
  local action="$2" rc
  case "$action" in
  ...
  aws "$@" 2>"$_AWS_ERRFILE"; rc=$?
  if [ "$rc" -ne 0 ] && grep -qiE 'AccessDenied|not authorized|UnauthorizedOperation|AuthorizationError|ExpiredToken|InvalidClientTokenId|Throttling|RequestLimitExceeded' "$_AWS_ERRFILE"; then
    echo "$1 $2" >>"$_DENIED_FILE"
    echo "[권한부족/오류] aws $1 $2 → 점검 불가(결과 신뢰 불가, 보고서 메타에 표기)" >&2
  fi
  return "$rc"
}

# 동일 호출 메모이즈(중복 describe 제거 → 호출수·스로틀 위험↓). 키=인자 전체 → 캐시파일.
aws_ro_cached(){
  local f; f="$_RO_CACHE_DIR/$(printf '%s' "$*" | cksum | tr -d ' \n')"
  [ -f "$f" ] || aws_ro "$@" >"$f"   # 최초 1회만 실제 호출(빈 결과도 파일로 캐시)
  cat "$f"
}
...
check_account(){
  ...
  kbuckets=$(aws_ro_cached s3api list-buckets --query 'Buckets[].Name' --output text)
  raw=""
...
check_resource(){
  ...
  buckets=$(aws_ro_cached s3api list-buckets --query 'Buckets[].Name' --output text)
  raw=""
  for b in $buckets; do
  ...
  rds_list=$(aws_ro_cached rds describe-db-instances --query 'DBInstances[].DBInstanceIdentifier' --output text)
  if [ -z "$rds_list" ]; then
...
if [ -n "$OUTDIR" ]; then
  ...
  # 보고서 TXT: [사전 정보] 헤더 + [종합] + emit_screen 블록 (was 양식 통일)
  { echo "진단 스크립트 시작"
    echo "================================================================"
    echo "[사전 정보]"
    echo "진단 대상    : ${TARGET_SYS}"
    echo "대상 계정    : ${ACCOUNT_ID}"
    echo "점검 리전    : ${REGION_EFF:-미지정}"
    echo "점검 분류    : CLOUD (AWS)    [전체 분류: WAS / DB / WEB / INFRA]"
    echo "진단 주체    : ${WHOAMI}"
    echo "점검 시각    : ${NOW}"
...
echo "================================================================"
printf "[종합] 총 %d개 | 양호 %d | 취약 %d | N/A %d\n" "$TOTAL" "$PASS_CNT" "$FAIL_CNT" "$NA_CNT"
if [ -s "$_DENIED_FILE" ]; then
  _dn=$(sort -u "$_DENIED_FILE" | wc -l)
  printf "[경고] 권한부족/오류로 점검 불가한 API %d종 — 결과 불완전 가능(보고서 헤더 참조)\n" "$_dn"
fi
if [ -n "$OUTDIR" ]; then
...
# 임시 파일 정리 + 종료코드 = 취약 개수(0=전부 양호, CI 게이팅용; 255 상한)
rm -f "$_AWS_ERRFILE" "$_DENIED_FILE" 2>/dev/null
rm -rf "$_RO_CACHE_DIR" 2>/dev/null
exit $(( FAIL_CNT > 255 ? 255 : FAIL_CNT ))
